...
class PickObjectTest1Action : public BT::AsyncActionNode {
 public:
  PickObjectTest1Action(const std::string& name,
  ...
  virtual BT::NodeStatus tick() override {
    ...
    RCLCPP_INFO(node_->get_logger(), "Pick object goal %f %f %f", pos.x, pos.y,
                pos.z);
    ...
    moveit::planning_interface::MoveGroupInterface move_group_eff(
        node_, "arm_end_effector");
    move_group_eff.setJointValueTarget(
        move_group_eff.getNamedTargetValues("open"));
    move_group_eff.move();
    ...
    move_group_eff.setJointValueTarget(
        move_group_eff.getNamedTargetValues("closed"));
    move_group_eff.move();
