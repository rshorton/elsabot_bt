...
class RobotCatGame {
 public:
  ...
  bool Init(double timeout_sec, double yaw_min, double yax_max, double yaw_step,
            double yaw_evade_step, double pitch_min, double pitch_max,
            double pitch_step, double pitch_dither_step);

  bool NextHeadPose(const robot_head_interfaces::msg::TrackStatus& track_status,
                    OrientationRPY& orient, std::string& action_desc);

  static double UNSET() { return DBL_MAX; };
