...
bool GameSettings::InitSettingsFilePath(std::string filename) {
  ...
  if (!dir) {
    cout << "Cannot initialize setting file path. Please env var GAME_DATA_DIR "
            "to the directory holding the game data/settings files."
         << std::endl;
    return false;
  }
  std::stringstream ss;
...
bool GameSettings::GetStringProperty(std::string prop, std::string& val) {
  if (settings_.contains(prop) && settings_[prop].is_string()) {
    val = settings_[prop];
    return true;
  }
  return false;
...
bool GameSettings::GetNumberProperty(std::string prop, int& val) {
  if (settings_.contains(prop) && settings_[prop].is_number()) {
    val = settings_[prop];
    return true;
  }
  return false;
...
bool GameSettings::GetBoolProperty(std::string prop, bool& val) {
  if (settings_.contains(prop) && settings_[prop].is_boolean()) {
    val = settings_[prop];
    return true;
  }
  return false;
...
bool GameSettings::GetStringMapProperty(
    std::string prop, std::map<std::string, std::string>& val) {
  if (settings_.contains(prop) && settings_[prop].is_object()) {
    // doesn't work
    // val = settings_[prop];
