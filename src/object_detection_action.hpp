...
// Singleton for subscribing to object detection message - shared by all
// ObjectDetection node instances
class ObjectDetectionROSNodeIf {
 public:
  ...
  static std::shared_ptr<ObjectDetectionROSNodeIf> instance(
      rclcpp::Node::SharedPtr node) {
    static std::shared_ptr<ObjectDetectionROSNodeIf> s{
        new ObjectDetectionROSNodeIf(node)};
    return s;
  }

  void update() { rclcpp::spin_some(node_); }
  ...
  ObjectDetectionROSNodeIf(rclcpp::Node::SharedPtr node)
    ...
    detected_obj_sub_ =
        node_->create_subscription<object_detection_msgs::msg::ObjectDescArray>(
            "/head/detected_objects", rclcpp::SystemDefaultsQoS(),
            std::bind(&ObjectDetectionROSNodeIf::detectCallback, this,
                      std::placeholders::_1));
  }
...
class ObjectDetectionAction : public BT::SyncActionNode {
 public:
  ObjectDetectionAction(const std::string& name,
  ...
  static BT::PortsList providedPorts() {
    return {BT::InputPort<std::string>("class"),
  ...
  virtual BT::NodeStatus tick() override {
    ...
    RCLCPP_INFO(node_if_->node_->get_logger(),
                "Check for objects: class= [%s], detected= [%d]",
                object_class.c_str(), node_if_->detected_);
    ...
    for (size_t i = 0; i < node_if_->objArray_.objects.size(); i++) {
      object_detection_msgs::msg::ObjectDesc& obj =
          node_if_->objArray_.objects[i];
      if (obj.track_status == "TRACKED" &&
          obj.name == object_class) {  // && obj.confidence >= min_confidence) {
        double sqdist = obj.position.point.x * obj.position.point.x +
                        obj.position.point.y * obj.position.point.y;
    ...
    if (closest != -1) {
      ...
      cout << "x " << node_if_->objArray_.objects[closest].position.point.x
           << ", y " << node_if_->objArray_.objects[closest].position.point.y
           << ", dist " << closest_dist << ", conf "
           << node_if_->objArray_.objects[closest].confidence << std::endl;
