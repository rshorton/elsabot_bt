...
class LLCoordToLocalUTM : public BT::SyncActionNode {
 ...
 public:
  LLCoordToLocalUTM(const std::string& name,
  ...
  static BT::PortsList providedPorts() {
    return {
        ...
        BT::InputPort<std::string>(
            "ll_kml_path"),  // the path to a path KML file with a list of
                             // coordinates. You can use Google Earth Pro to
                             // create.
  ...
  virtual BT::NodeStatus tick() override {
    ...
    if (!getInput<std::string>("ll_poses", ll_poses)) {
      if (!getInput<std::string>("ll_kml_path", ll_kml_path)) {
        throw BT::RuntimeError(
            "LLCoordToLocalUTM requires ll_poses or ll_krl_path");
      }
    }
    ...
    if (!poses.size()) {
      RCLCPP_ERROR(node_->get_logger(),
                   "LLCoordToLocalUTM failed to prepare pose list");
      return BT::NodeStatus::FAILURE;
    }

    auto client =
        node_->create_client<robot_localization::srv::FromLL>("fromLL");

    // if no server is present, fail after N seconds
    if (!client->wait_for_service(std::chrono::seconds(20))) {
      RCLCPP_ERROR(node_->get_logger(),
                   "LL to UTM action service not available after waiting");
      return BT::NodeStatus::FAILURE;
    }

    auto ok = true;
    auto utm_poses =
        std::make_shared<std::vector<geometry_msgs::msg::PoseStamped>>();

    for (const auto& pose : poses) {
      auto request =
          std::make_shared<robot_localization::srv::FromLL::Request>();
      request->ll_point.latitude = pose.x;
      request->ll_point.longitude = pose.y;
      request->ll_point.altitude = 0.0;

      RCLCPP_INFO(node_->get_logger(),
                  "Sending LL to UTM request for point: %f, %f", pose.x,
                  pose.y);
      ...
      RCLCPP_INFO(node_->get_logger(), "local utm: %f, %f ", map_point.x,
                  map_point.y);
  ...
  // Parse the coordinates from a KML file created by exporting a Path from
  // Google Earth Pro
  bool read_poses_from_kml(const std::string& ll_kml_path,
                           std::vector<Pose2D>& poses) {
    ...
    if (std::regex_search(s, match, rgx)) {
      ...
      RCLCPP_INFO(node_->get_logger(), "Parsed %ld coords from KML",
                  poses.size());
      return true;
    }
    return false;
