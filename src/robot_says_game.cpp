...
RobotSaysGame* RobotSaysGame::GetRobotSaysGame() { return game; }
...
RobotSaysGame::RobotSaysGame()
    : easy_(true), level_start_(0), level_end_(0), step_index_(0) {
  ...
  poses_all_ = {OnHip,    ArmOut, ArmToSide, TShoulder,
                TStomach, AHead,  OHead /*, TNeck*/};

  levels_ = {SideAny, Left, Right, LeftAndRight};
  level_desc_ = {"Either left or right", "Left hand only", "Right hand only",
...
int RobotSaysGame::NextPass(bool reset, std::string& level_desc) {
  ...
  shuffle(random_poses_.begin(), random_poses_.end(),
...
int RobotSaysGame::NextStep(string& pose_name_l, string& pose_name_r,
                            string& pose_lr_check, string& pose_speech,
                            int32_t& step_index) {
  pose_name_l = "";
  pose_name_r = "";
  pose_speech = "";
...
int32_t RobotSaysGame::GetMinDifficulty() { return 0; }

int32_t RobotSaysGame::GetMaxDifficulty() { return levels_.size() - 1; }
