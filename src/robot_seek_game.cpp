...
RobotSeekGame* RobotSeekGame::GetRobotSeekGame() { return game; }
...
RobotSeekGame::RobotSeekGame() : idx_cur_(0), idx_start_(0), bFirst_(false) {}
...
void RobotSeekGame::GetPointInSameDirection(SearchPose p0, SearchPose p1,
                                            ...
                                            SearchPose& best, double& angleDiff,
                                            double& angleTo) {
  ...
  cout << "a0= " << a0 << "(" << rad_to_deg(a0) << "), a1= " << a1 << "("
       << rad_to_deg(a1) << "), yaw= " << yaw << "(" << rad_to_deg(yaw) << ")"
       << endl;
...
bool RobotSeekGame::Init(rclcpp::Node::SharedPtr node, std::string datapath,
  ...
  for (json::iterator it = game_data["search_poses"].begin();
       it != game_data["search_poses"].end(); ++it) {
    SearchPose sp;
    ...
    poses_.push_back(sp);
    cout << "x= " << sp.x << ", y= " << sp.y << ", yaw= " << sp.yaw
         << ", spin= " << sp.spin << ", scan= " << sp.scan << endl;
  }

  // Read the game boundary
  boundary_.clear();
  cout << "Game boundary points: " << endl;
  for (json::iterator it = game_data["boundary"].begin();
       it != game_data["boundary"].end(); ++it) {
    Position pos;
  ...
  for (auto& it : pose_tmp) {
    cout << "x= " << it.x << ", y= " << it.y << ", dist= " << it.initial_dist
         << endl;
  }
  ...
  double angleDiff;
  double angleTo;
  GetPointInSameDirection(pose_tmp[0], pose_tmp[1], init_x, init_y, init_yaw,
                          best, angleDiff, angleTo);
  ...
  cout << "Selected initial pose x,y= " << poses_[idx_start_].x << ", "
       << poses_[idx_start_].y << endl;
  idx_cur_ = idx_start_;
  bFirst_ = true;
  ...
  // direction as the arrival direction to the start pose from the current robot
  // position.
  GetPointInSameDirection(poses_[prev_idx], poses_[next_idx],
                          poses_[idx_start_].x, poses_[idx_start_].y, angleTo,
                          best, angleDiff, angleTo);
...
bool RobotSeekGame::NextSearchPose(double& x, double& y, double& yaw,
                                   bool& spin_at_goal, bool& scan_at_goal,
                                   int& index) {
  if (bFirst_) {
    bFirst_ = false;
  } else {
  ...
  for (int i = 0, j = vertices - 1; i < vertices; j = i++) {
    if (((boundary_[i].y > pos.y) != (boundary_[j].y > pos.y)) &&
        (pos.x < (boundary_[j].x - boundary_[i].x) * (pos.y - boundary_[i].y) /
                         (boundary_[j].y - boundary_[i].y) +
                     boundary_[i].x)) {
      inside = !inside;
    }
  }
  RCLCPP_INFO(node_->get_logger(), "InBoundary, ck: (%f, %f), inside: %d",
              pos.x, pos.y, inside);
...
void RobotSeekGame::TestBoundaryCheck() const {
  Position pnts[] = {Position(-0.038488, -2.554656, 0.0),
