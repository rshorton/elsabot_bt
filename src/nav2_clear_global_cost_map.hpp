...
class Nav2ClearGlobalCostMap : public BT::SyncActionNode {
 public:
  Nav2ClearGlobalCostMap(const std::string& name,
  ...
  virtual BT::NodeStatus tick() override {
    ...
    auto client = node_->create_client<nav2_msgs::srv::ClearEntireCostmap>(
        "global_costmap/clear_entirely_global_costmap");

    // if no server is present, fail after 5 seconds
    if (!client->wait_for_service(std::chrono::seconds(20))) {
      RCLCPP_ERROR(node_->get_logger(),
                   "Action service not available after waiting");
    ...
    RCLCPP_INFO(node_->get_logger(),
                "Sending request to clear global cost map");
