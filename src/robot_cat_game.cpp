...
bool RobotCatGame::Init(double timeout_sec, double yaw_min, double yaw_max,
                        double yaw_step, double yaw_evade_step,
                        double pitch_min, double pitch_max, double pitch_step,
                        double pitch_dither_step) {
...
bool RobotCatGame::NextHeadPose(
    const robot_head_interfaces::msg::TrackStatus& ts, OrientationRPY& orient,
    std::string& action_desc) {
  RCLCPP_DEBUG(rclcpp::get_logger("elsabot_bt"),
               "NextHeadPos: track status, tracking= %d, moving= %d, pan= %f, "
               "tilt= %f, rot= %f, bb_x_min= %f, bb_y_min= %f, bb_x_max= %f, "
               "bb_y_max= %f, conf= %f, yar_dir= %d, move_to_limit= %d",
               ts.tracking, ts.moving, ts.pan_angle, ts.tilt_angle,
               ts.rotate_angle, ts.object.bb_x_min, ts.object.bb_y_min,
               ts.object.bb_x_max, ts.object.bb_y_max, ts.object.confidence,
               yaw_dir_, move_to_limit_);
  ...
  if (!ts.moving) {
    ...
    if (ts.object.bb_x_max > 0.4 && ts.object.bb_x_min < 0.6) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("elsabot_bt"),
                  "NextHeadPos: cat is close, move now");
      action_desc = "moving";
    ...
    if (move || move_to_limit_) {
      ...
      if (orient.y > yaw_max_ || orient.y < yaw_min_) {
        yaw_dir_ *= -1;

        RCLCPP_INFO(rclcpp::get_logger("elsabot_bt"),
                    "NextHeadPos: at yaw limit, reverse yaw dir");
        move_to_limit_ = false;
      }

      // Zig-zag the pitch as the head moves in yaw direction.
      orient.p += pitch_step_ * pitch_dir_;
      if (orient.p > pitch_max_ || orient.p < pitch_min_) {
        pitch_dir_ *= -1;

        RCLCPP_INFO(rclcpp::get_logger("elsabot_bt"),
                    "NextHeadPos: at pitch limit, reverse pitch dir");
      }

      time_last_move_ = std::chrono::steady_clock::now();

    } else {
      // If cat not close enough, then after N seconds, sweep the head to the
      // farther limit.
      std::chrono::duration<double, std::milli> since_last_move =
          std::chrono::steady_clock::now() - time_last_move_;
      ...
      if (since_last_move.count() > timeout_sec_) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("elsabot_bt"),
                    "NextHeadPos: timeout, start move to limit, dir= %d",
                    yaw_dir_);
      } else {
        action_desc = "waiting";
      }

      std::chrono::duration<double, std::milli> since_last_small_step =
          std::chrono::steady_clock::now() - time_last_small_step_;
      if (since_last_small_step.count() > 500) {
        // Move the head slowly in the pitch direction while waiting for the cat
        // to move.
