...
class DetectionSelectAction : public BT::SyncActionNode {
 public:
  DetectionSelectAction(const std::string& name,
  ...
  virtual BT::NodeStatus tick() override {
    ...
    if (proc->Select(obj_class, ObjDetProc::SelectionMetric::closest, token,
      ...
      if (proc->GetSelected(det)) {
        RCLCPP_INFO(node->get_logger(),
                    "SelectObjectAction: GetSelected returned: id: [%lu], "
                    "dist: [%f], x,y,z,yaw: %f, %f, %f, %f, token: [%s]",
                    det.id, det.dist, det.pos.x, det.pos.y, det.pos.z, det.yaw,
                    det.token.c_str());
        ...
        RCLCPP_INFO(node->get_logger(),
                    "SelectObjectAction: GetSelected failed");
      }
    } else {
