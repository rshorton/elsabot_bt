...
// Singleton for publishing the pose update - shared by all
// PublishPositionAsGoalAction instances
class PublishPositionAsGoalActionROSNodeIf {
 public:
  PublishPositionAsGoalActionROSNodeIf(
      PublishPositionAsGoalActionROSNodeIf const&) = delete;
  PublishPositionAsGoalActionROSNodeIf& operator=(
      PublishPositionAsGoalActionROSNodeIf const&) = delete;

  static std::shared_ptr<PublishPositionAsGoalActionROSNodeIf> instance(
      rclcpp::Node::SharedPtr node) {
    static std::shared_ptr<PublishPositionAsGoalActionROSNodeIf> s{
        new PublishPositionAsGoalActionROSNodeIf(node)};
    return s;
  }

  void pubGoalUpdate(const Position& position) {
    RCLCPP_INFO(node_->get_logger(), "Pub goal update: x,y= %f, %f", position.x,
                position.y);
...
class PublishPositionAsGoalAction : public BT::SyncActionNode {
 public:
  PublishPositionAsGoalAction(const std::string& name,
