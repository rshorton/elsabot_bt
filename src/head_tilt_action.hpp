...
// Singleton for publishing the HeadTilt control state - shared by all
// HeadTiltActionNode instances
class HeadTiltActionROSNodeIf {
 public:
  ...
  static std::shared_ptr<HeadTiltActionROSNodeIf> instance(
      rclcpp::Node::SharedPtr node) {
    static std::shared_ptr<HeadTiltActionROSNodeIf> s{
        new HeadTiltActionROSNodeIf(node)};
    return s;
  }

  void setHeadTiltState(int32_t angle, int32_t dwell_ms) {
    RCLCPP_INFO(node_->get_logger(), "Set head tilt: angle= %d, dwell_ms= %d",
                angle, dwell_ms);
 ...
 private:
  HeadTiltActionROSNodeIf(rclcpp::Node::SharedPtr node) : node_(node) {
    head_tilt_publisher_ =
        node_->create_publisher<robot_head_interfaces::msg::HeadTilt>(
            "/head/tilt", 2);
  }
  rclcpp::Node::SharedPtr node_;
  rclcpp::Publisher<robot_head_interfaces::msg::HeadTilt>::SharedPtr
      head_tilt_publisher_;
};
 ...
 public:
  ...
  static BT::PortsList providedPorts() {
    return {BT::InputPort<std::string>("angle"),
            BT::InputPort<std::string>("dwell_ms")};
  }
