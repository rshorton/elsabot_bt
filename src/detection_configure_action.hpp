...
class DetectionConfigureAction : public BT::SyncActionNode {
 public:
  DetectionConfigureAction(const std::string& name,
  ...
  virtual BT::NodeStatus tick() override {
    ...
    RCLCPP_INFO(node->get_logger(),
                "Configure object detection processor [%s] with class-conf-map "
                "[%s], min_det_count [%d], "
                "drop_det_count [%d], det_timeout [%d], pub_topic [%s]",
                det.c_str(), class_conf_mapping.c_str(), min_det_count,
                drop_det_count, det_timeout, pub_topic.c_str());

    if (!proc->ObjDetProc::Configure(objects_to_det, min_det_count,
                                     drop_det_count, det_timeout, pub_topic)) {
      return BT::NodeStatus::FAILURE;
    }
