...
class RobotFindInitAction : public BT::SyncActionNode {
 public:
  RobotFindInitAction(const std::string& name,
  ...
  virtual BT::NodeStatus tick() override {
    ...
    if (!dir) {
      throw BT::RuntimeError(
          "Please set env var GAME_DATA_DIR to the directory holding the game "
          "data/settings files.");
    }
    ...
    if (game->Init(ss.str(), positions)) {
      ...
      if (!game->InitGameRound(object_type, random_order)) {
        throw BT::RuntimeError(
            "invalid object type, not supported in find game json");
      }
