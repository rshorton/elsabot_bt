...
RobotFindGame* RobotFindGame::GetRobotFindGame() { return game; }
...
bool RobotFindGame::InitGameRound(std::string object_type, bool random) {
  ...
  if (random) {
    ...
    shuffle(object_names_round_.begin(), object_names_round_.end(),
            std::default_random_engine(seed));
  }

  std::cout << "Object list for round: ";
  for (auto it = object_names_round_.begin(); it != object_names_round_.end();
       it++) {
    std::cout << *it << " ";
  }
  std::cout << endl;
...
bool RobotFindGame::ProcessPositions() {
  ...
  for (auto& it : positions_) {
    ...
    if (bFirst) {
      ...
      cout << "position: " << it.first << ", min_dist: " << min_dist
           << ", dist: " << dist << std::endl;
  ...
  cout << "min_dist: " << min_dist << ", pos_threshold: " << pos_threshold_
       << std::endl;
  return true;
}
...
bool RobotFindGame::CheckRound(double x, double y) {
  if (idx_cur_ < (int)object_names_round_.size()) {
    ...
    std::cout << "CheckRound: dist= " << dist << ", at_pos_cnt= " << at_pos_cnt_
              << std::endl;
    if (at_pos_cnt_ >= 3) {
      return true;
    }
...
bool RobotFindGame::GetPositions(
    std::vector<RobotFindGame::position_list_item>& positions) {
  for (auto& it : positions_) {
...
bool RobotFindGame::GetObjectTypeNames(std::string object_type,
...
bool RobotFindGame::Load() {
  ...
  // Game data will contain x,y = 0,0 initially.  The x,y position for each item
  // needs to be specified by setting-up the game.
  positions_.clear();
  objects_.clear();
  ...
  // The positions array holds a list of objects which specify the floor
  // locations of the number cards.  It also implicitly specifies the list of
  // number objects.
  for (auto& it : game_data["positions"]) {
    ...
    cout << "name= " << name << ", x= " << pos.x << ", y= " << pos.y << endl;
  }

  if (game_data.contains("alt_objects") &&
      game_data["alt_objects"].is_array()) {
    ...
    for (auto& it : game_data["alt_objects"]) {
      if (it.is_object()) {
        ...
        for (auto& it2 : it["objects"]) {
          ...
          cout << "type: " << obj_type << ", " << name << ", at pos: " << pos
               << endl;
        }
      }
    }
