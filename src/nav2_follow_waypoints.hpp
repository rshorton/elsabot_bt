...
class Nav2FollowWayoints : public BT::AsyncActionNode {
 ...
 public:
  Nav2FollowWayoints(const std::string& name,
                     const BT::NodeConfiguration& config)
      : BT::AsyncActionNode(name, config), _aborted(false) {}

  static BT::PortsList providedPorts() {
    return {BT::InputPort<pose_list>("poses"),
  ...
  virtual BT::NodeStatus tick() override {
    ...
    RCLCPP_INFO(node_->get_logger(), "Navigating thru %ld poses",
                poses->size());
    ...
    if (rclcpp::spin_until_future_complete(node_, goal_handle_future) !=
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "Nav2 nav through poses send goal call failed");
      return BT::NodeStatus::FAILURE;
    }

    rclcpp_action::ClientGoalHandle<
        nav2_msgs::action::FollowWaypoints>::SharedPtr goal_handle =
        goal_handle_future.get();
    if (!goal_handle) {
      RCLCPP_ERROR(node_->get_logger(),
                   "Nav2 nav through poses goal was rejected by server");
      return BT::NodeStatus::FAILURE;
    }
    ...
    rclcpp_action::ClientGoalHandle<
        nav2_msgs::action::FollowWaypoints>::WrappedResult wrapped_result =
        result_future.get();
