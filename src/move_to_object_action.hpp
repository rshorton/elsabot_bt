...
class MoveToObjectAction : public BT::AsyncActionNode {
 public:
  MoveToObjectAction(const std::string& name,
  ...
  virtual BT::NodeStatus tick() override {
    ...
    while (!_aborted && rclcpp::ok()) {
      ...
      RCLCPP_INFO(node_->get_logger(),
                  "relative obj pos: x,y: %f, %f, dist: %f", obj.x, obj.y,
                  dist);
      ...
      if (xVel == 0.0 &&
          (rot_vel == 0.0 ||
           (last_rot_vel != std::numeric_limits<double>::infinity() &&
            signbit(rot_vel) != signbit(last_rot_vel)))) {
        atPos = true;
        rot_vel = 0.0;
      }
      ...
      RCLCPP_INFO(
          node_->get_logger(),
          "angle: %f, rot_vel: %f, last_rot_vel: %f, xVel: %f, atPos: %d",
