...
class Nav2ClientLoop : public BT::AsyncActionNode {
 ...
 public:
  Nav2ClientLoop(const std::string& name, const BT::NodeConfiguration& config)
      : BT::AsyncActionNode(name, config), _aborted(false) {}

  static BT::PortsList providedPorts() {
    return {
        BT::InputPort<std::string>("loop_end_pos"),
  ...
  virtual BT::NodeStatus tick() override {
    ...
    auto goal_update_publisher =
        node_->create_publisher<geometry_msgs::msg::PoseStamped>("/goal_update",
                                                                 1);
    ...
    if (!getInput<std::string>("loop_intermediate_pos",
                               loop_intermediate_pos_str)) {
    ...
    Pose2D loop_intermediate_pos =
        BT::convertFromString<Pose2D>(loop_intermediate_pos_str);
    Pose2D goal;

    // Get the current state and determine whether the robot is currently at the
    // loop end position. If so, then set the initial goal to be at an
    // intermeditate point along the loop.  Otherwise make the initial goal the
    // loop end point.
    {
      double x, y, z, yaw;
      ...
      for (int t = 0; t < 20; t++) {
        ...
        RCLCPP_INFO(node_->get_logger(),
                    "Robot pose not available.  Waiting...");
      ...
      if (!got_pose) {
        RCLCPP_INFO(node_->get_logger(),
                    "Robot pose not available.  Failing, cannot determine "
    ...
    goal_msg.pose.pose.orientation =
        orientationAroundZAxis(goal.yaw / 180.0 * M_PI);
    goal_msg.behavior_tree = behavior_tree;

    RCLCPP_INFO(
        node_->get_logger(), "orientation %f %f %f %f",
    ...
    while (true) {
      auto result =
          rclcpp::spin_until_future_complete(node_, result_future, timeout);
      if (result == rclcpp::FutureReturnCode::TIMEOUT) {
        if (_aborted) {
          RCLCPP_INFO(node_->get_logger(),
                      "waiting for nav to complete...aborted");
          break;
        }
        ...
        if (++temp_cnt > 20) {
          ...
          if (position_state == PositionState::MOVE_AWAY_FROM_GOAL) {
            auto dist_from_loop_end =
                calc_distance(x, y, loop_end_pos.x, loop_end_pos.y);
            if (dist_from_loop_end > 2.0) {
              ...
              RCLCPP_INFO(node_->get_logger(),
                          "Published new goal for the loop end");
            } else {
              RCLCPP_INFO(node_->get_logger(),
                          "Waiting to publish loop end goal (%f)",
                          dist_from_loop_end);
            }
          }
        }
        ...
        bSuccess = true;
        RCLCPP_INFO(node_->get_logger(),
                    "waiting for nav to complete...success");
        break;
      }
    }
