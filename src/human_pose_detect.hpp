...
// Singleton for subscribing to human pose detection message - shared by all
// HumanPoseDetect node instances
class HumanPoseDetectROSNodeIf {
 public:
  ...
  static std::shared_ptr<HumanPoseDetectROSNodeIf> instance(
      rclcpp::Node::SharedPtr node) {
    static std::shared_ptr<HumanPoseDetectROSNodeIf> s{
        new HumanPoseDetectROSNodeIf(node)};
    return s;
  }
 ...
 private:
  HumanPoseDetectROSNodeIf(rclcpp::Node::SharedPtr node)
    ...
    detected_pose_sub_ =
        node_->create_subscription<robot_head_interfaces::msg::DetectedPose>(
            "/head/detected_pose", rclcpp::SystemDefaultsQoS(),
            std::bind(&HumanPoseDetectROSNodeIf::poseCallback, this,
                      std::placeholders::_1));
  }
  ...
  void poseCallback(robot_head_interfaces::msg::DetectedPose::SharedPtr msg) {
    ...
    RCLCPP_INFO(node_->get_logger(), "Got pose callback [%s], [%s]",
                cur_pose_left_.c_str(), cur_pose_right_.c_str());
  }

  rclcpp::Subscription<robot_head_interfaces::msg::DetectedPose>::SharedPtr
      detected_pose_sub_;
};
...
class HumanPoseDetect : public BT::SyncActionNode {
 public:
  ...
  static BT::PortsList providedPorts() {
    return {BT::InputPort<std::string>("expected_pose_left"),
  ...
  virtual BT::NodeStatus tick() override {
    ...
    RCLCPP_INFO(node_if_->node_->get_logger(),
                "Check for poses: person_det= [%d], Compare: [%s],  L [%s], R "
                "[%s], current L [%s], R[%s]",
                node_if_->detected_, pose_lr_check.c_str(),
                expected_left.c_str(), expected_right.c_str(),
                node_if_->cur_pose_left_.c_str(),
                node_if_->cur_pose_right_.c_str());

    // Return success if both of the expected left/right poses are seen.  Either
    // or both can be required.  If neither l/r poses are specified, then
    // success if a person is detected at all.
    if (node_if_->detected_ && node_if_->num_points_ >= min_points) {
      ...
      if (expected_left.length() != 0 &&
          expected_left.compare(node_if_->cur_pose_left_) == 0) {
        okL = true;
      }
      if (expected_right.length() != 0 &&
          expected_right.compare(node_if_->cur_pose_right_) == 0) {
        okR = true;
      }
