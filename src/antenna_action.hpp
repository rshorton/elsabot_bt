...
// Single for publishing the antenna control state - shared by all
// AntennaActionNode instances
class AntennaActionROSNodeIf {
 public:
  ...
  static std::shared_ptr<AntennaActionROSNodeIf> instance(
      rclcpp::Node::SharedPtr node) {
    static std::shared_ptr<AntennaActionROSNodeIf> s{
        new AntennaActionROSNodeIf(node)};
    return s;
  }

  void setAntennaState(uint32_t rate, uint32_t intensity,
                       std::string left_blink_pattern,
                       std::string right_blink_pattern) {
    RCLCPP_INFO(node_->get_logger(),
                "Set Antenna: rate= %d, intensity= %d, left pattern= %s, right "
                "pattern= %s",
                rate, intensity, left_blink_pattern.c_str(),
                right_blink_pattern.c_str());
 ...
 private:
  AntennaActionROSNodeIf(rclcpp::Node::SharedPtr node) : node_(node) {
    antenna_publisher_ =
        node_->create_publisher<robot_head_interfaces::msg::Antenna>(
            "/head/antenna", 2);
  }
  rclcpp::Node::SharedPtr node_;
  rclcpp::Publisher<robot_head_interfaces::msg::Antenna>::SharedPtr
      antenna_publisher_;
};
...
class AntennaAction : public BT::SyncActionNode {
 public:
  ...
  static BT::PortsList providedPorts() {
    return {BT::InputPort<std::string>("left_blink_pattern"),
            BT::InputPort<std::string>("right_blink_pattern"),
            BT::InputPort<uint32_t>("rate"),
            BT::InputPort<uint32_t>("intensity")};
  }
  ...
  virtual BT::NodeStatus tick() override {
    ...
    node_if_->setAntennaState(rate, intensity, left_blink_pattern,
                              right_blink_pattern);
